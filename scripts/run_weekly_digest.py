...
def main() -> None:
    ...
    try:
        report = digest_service.generate_report(user_id=args.user_id, start=start, end=end, force=args.force)
    except ValueError as err:
        logging.info("Digest skipped: %s", err)
        return

    if args.dry_run:
        print(report.email_body or "")
        logging.info("Dry run complete; report not sent.")
        return
    ...
    with get_session() as session:
        ...
        if refreshed:
            ...
            session.commit()
    logging.info("Weekly digest generated and sent for user %s.", args.user_id)
